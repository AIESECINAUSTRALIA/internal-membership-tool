...
export function AddMemberDialog({ open, reference, actor, permissions, onClose, onAdded }: Props) {
  ...
  const lcOptions = addableLcs(actor, permissions, reference.lcs)
  const lcFixed = lcOptions.length === 1
  const lc = lcOptions.find((l) => l.id === lcChoice) ?? (lcFixed ? lcOptions[0] : undefined)
  const positionOptions = lc ? addablePositions(actor, permissions, reference.positions, lc) : []
  const position =
    positionOptions.find((p) => p.key === positionChoice) ??
    (positionOptions.length === 1 ? positionOptions[0] : undefined)
  const functionOptions = position ? addableFunctions(actor, permissions, reference.functions, position) : []
  const fn =
  ...
  const valid = !emailError && !startError && !endError && !nameError && lc && position && fn
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ pt: 1 }} component="form" noValidate onSubmit={(e) => { e.preventDefault(); void submit(false) }} id="add-member-form">
            ...
            {lcFixed ? (
              ...
              <TextField select label="LC" required value={lc?.id ?? ''} onChange={(e) => { setLcChoice(e.target.value); setPositionChoice(''); setFunctionChoice('') }} error={submitted && !lc} helperText={submitted && !lc ? 'Choose an LC.' : ' '} fullWidth>
                {lcOptions.map((l) => (
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <TextField select label="Position" required value={position?.key ?? ''} onChange={(e) => { setPositionChoice(e.target.value); setFunctionChoice('') }} error={submitted && !position} helperText={submitted && !position ? 'Choose a position.' : ' '} disabled={!lc} fullWidth>
                {positionOptions.map((p) => (
                  <MenuItem key={p.key} value={p.key}>{p.label}</MenuItem>
                ))}
              </TextField>
              <TextField select label="Function" required value={fn?.key ?? ''} onChange={(e) => setFunctionChoice(e.target.value)} error={submitted && !fn} helperText={submitted && !fn ? 'Choose a function.' : ' '} disabled={!position} fullWidth>
